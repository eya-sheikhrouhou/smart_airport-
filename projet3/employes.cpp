...
bool Employes::modifier()
{
    ...
    QString res3=QString::number(nb_heures_sup);
    QString res4=QString::number(taches);
    query.prepare("UPDATE EMPLOYES SET CIN=:res,NOM=:nom,PRENOM=:prenom,POSTE=:poste,"
                  "SALAIRE=:res1,NB_HEURES_NOR=:res2,NB_heures_SUP=:res3,TACHES=:res4 WHERE CIN=:cin");
   query.bindValue(":cin",res);
    query.bindValue(":nom",nom);
    query.bindValue(":prenom",prenom);
    query.bindValue(":poste",poste);
